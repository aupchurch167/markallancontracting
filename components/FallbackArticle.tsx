  ...
  return (
    <div>
      {body.map((block, i) => {
        if ('h2' in block) {
          return (
            <h2 key={i} className="mt-10 text-2xl font-bold text-navy">
              {block.h2}
            </h2>
          );
        }
        ...
        if ('ul' in block) {
          return (
            <ul key={i} className="mb-4 list-disc space-y-1.5 pl-5 text-stone-600">
              {block.ul.map((li, j) => (
                <li key={j}>{li}</li>
              ))}
            </ul>
        ...
        return (
          <ol key={i} className="mb-4 list-decimal space-y-1.5 pl-5 text-stone-600">
            {block.ol.map((li, j) => (
              <li key={j}>{li}</li>
            ))}
          </ol>
